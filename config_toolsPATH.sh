# 紀錄各個程式的絕對路徑，如果需要更改就再相對應的程式名稱底下修改路徑。
# 交給 exportPATH.sh 執行，將這些路徑逐一 export 到 PATH 當中。

# SRA tools
/opt/ohpc/Taiwania3/pkg/biology/SRAToolkit/sratoolkit_v3.0.3/bin/
# Trimgalore
/opt/ohpc/Taiwania3/pkg/biology/TrimGalore/TrimGalore_v0.6.3/
# CutAdapt
/opt/ohpc/Taiwania3/pkg/biology/TrimGalore/TrimGalore_v0.6.3/bin
# FastQC
/opt/ohpc/Taiwania3/pkg/biology/FastQC/FastQC_v0.11.9
# Bowtie2
/opt/ohpc/Taiwania3/pkg/biology/BOWTIE/bowtie2_v2.4.2/
# STAR
/opt/ohpc/Taiwania3/pkg/biology/STAR/STAR_v2.7.9a/bin/Linux_x86_64/
# Samtools
/opt/ohpc/Taiwania3/pkg/biology/SAMTOOLS/SAMTOOLS_v1.13/bin/
# DeepTools
/opt/ohpc/Taiwania3/pkg/biology/deepTools/deepTools_v3.3.1/bin/
# BBduck
/opt/ohpc/Taiwania3/pkg/biology/BBTools/BBTools_v37.62/bin/
# HTseq
/opt/ohpc/Taiwania3/pkg/biology/Htseq/Htseq_v0.13.5/bin/

